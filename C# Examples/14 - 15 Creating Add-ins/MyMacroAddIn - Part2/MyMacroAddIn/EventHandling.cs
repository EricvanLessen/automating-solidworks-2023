...
namespace MyMacroAddIn
{
    ...
    public class PartEventHandler : DocumentEventHandler
    {
        ...
        override public bool AttachEventHandlers()
        {
            ...
            doc.FileSaveAsNotify2 += new DPartDocEvents_FileSaveAsNotify2EventHandler(OnFileSaveAs);
            ConnectModelViews();
        ...
        public int OnFileSaveAs(string filename)
        {
            CustomProps cp = new CustomProps();
            string propName = "Description";
            if (cp.PropExists((ModelDoc2)doc, propName))
            {
                return 0;  //continue the save action
            }
            else
            {
                iSwApp.SendMsgToUser2("Please add " + propName
                    + " property before saving.", 
                    (int)swMessageBoxIcon_e.swMbStop, (int)swMessageBoxBtn_e.swMbOk);
                return 1; //stop the save action
            }
        }
